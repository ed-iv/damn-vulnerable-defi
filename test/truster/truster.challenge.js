...
describe("[Challenge] Truster", function () {
	...
	it("Execution", async function () {
		const BreakerFactory = await ethers.getContractFactory("TrustBreaker");
		const breaker = await BreakerFactory.connect(player).deploy(
			pool.address,
			token.address
		);

		await breaker.connect(player).attack();
	});
